...
- 多条关注任务时，上方逐条展示关注任务，底部固定展示进行中合并项；只有底部合并项可以展开/收起完整列表。
...
- 设置窗口：隐私模式、路径隐藏、标题隐藏、压缩态隐私模式、悬浮岛透明度、任务完成通知、自动确认、鼠标穿透。
...
- 任务完成通知默认关闭；开启后只发送来源、完成状态和按隐私设置处理后的任务标题，不发送 prompt、回复正文、完整工具输入输出、完整命令或 patch。
...
- 支持任务完成通知：
  - 用户打开后请求系统通知权限。
  - 任务从非完成态进入完成态时发送一条系统通知，并使用系统通知声音。
  - 同一完成事件只通知一次。
- 支持完成任务自动确认：

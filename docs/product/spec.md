...
- 设置窗口：隐私模式、路径隐藏、标题隐藏、压缩态隐私模式、悬浮岛透明度、关键状态通知、自动确认、鼠标穿透。
...
- 提供隐私模式：
  - 隐藏项目路径。
  - 隐藏任务标题。
  - 压缩态只显示来源和状态；展开列表仍按路径和标题隐私设置展示。
- 关键状态通知默认关闭；开启后只发送来源、等待处理/失败/完成状态和按隐私设置处理后的任务标题，不发送 prompt、回复正文、完整工具输入输出、完整命令或 patch。
...
- 支持关键状态通知：
  - 用户打开后请求系统通知权限。
  - 任务进入 `waiting-user`、`failed` 或 `completed` 时发送一条系统通知，并使用系统通知声音。
  - 同一关键状态事件只通知一次；应用启动或重新加载已有任务时不回放历史通知。
- 支持完成任务自动确认：

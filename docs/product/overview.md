...
- 详情态：展示单任务元信息、最近事件、等待原因或错误摘要，并提供打开、复制和手动清除异常状态的轻量操作。
- 面板动作：展开面板右上角提供诊断、设置和退出应用入口；设置、诊断和后续复杂功能打开独立桌面窗口。

压缩态中，`waiting-user`、`failed`、`completed`、`stale` 这类需要关注的任务逐条展示；`discovering`、`running`、`thinking`、`tool-running` 合并为“N 个任务进行中”。多条关注任务没有进行中任务时，底部仍保留“X 条任务需要关注”的完整列表入口。`paused` 表示用户主动中断，默认归档隐藏，不继续占用任务列表；后续同一会话恢复运行时重新显示。底部/单行右侧入口在收起时显示“显示全部任务”，展开后显示“收起全部任务”。
...
关键状态通知默认关闭。用户打开后，任务进入 `waiting-user`、`failed` 或 `completed` 时发送一条系统通知，并使用设置中选择的系统默认音、macOS 内置声音或无声；任务停留在同一关键状态期间，后续同类状态事件不重复通知，启动或重新加载已有任务时不回放历史通知。通知不包含 prompt、回复正文、完整工具输入输出、完整命令或 patch。

自动确认开启后，未手动确认的 `completed` 任务会在 5 分钟、15 分钟、30 分钟或 1 小时后自动归档；`paused`、`waiting-user` 和 `failed` 不会被自动确认。

详情态的“清除任务状态”用于异常兜底：当真实任务已经结束、失败或不再重要，但 hook 状态仍停留在进行中、等待或失败时，用户可以手动把当前 `task.id + updatedAt` 对应的展示状态按“完成且已确认”归档。该动作不修改 agent、hook spool 或原始事件；后续同一任务有新状态事件时会重新显示。

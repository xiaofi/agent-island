...
关键状态通知默认关闭。用户打开后，任务进入 `waiting-user`、`failed` 或 `completed` 时发送一条系统通知，并使用设置中选择的系统默认音、macOS 内置声音或无声；任务停留在同一关键状态期间，后续同类状态事件不重复通知，启动或重新加载已有任务时不回放历史通知，通知内容遵循标题隐私设置。

...
- 关键状态通知默认关闭。开启后，Agent Island 会请求系统通知权限；通知只包含来源、等待处理/失败/完成状态和按隐私设置处理后的任务标题，不包含 prompt、回复正文、完整工具输入/输出、完整 shell command、完整 patch 或 transcript 内容。通知声音可在设置中选择系统默认、若干 macOS 内置声音或无声。

...
- 不提供隐藏项目路径、隐藏任务标题或压缩态隐私模式设置；隐私边界依赖本地运行、最小化采集和不保存敏感正文。
- 关键状态通知默认关闭。开启后，Agent Island 会请求系统通知权限；通知只包含来源、等待处理/失败/完成状态和任务标题，不包含 prompt、回复正文、完整工具输入/输出、完整 shell command、完整 patch 或 transcript 内容。通知声音可在设置中选择系统默认、若干 macOS 内置声音或无声。
...
关键状态通知使用 Tauri notification 插件接入系统通知。用户首次打开“关键状态通知”时，前端只负责请求系统通知权限；实际发送由 Rust task watcher 在任务状态同步时统一执行，避免多窗口重复发送。权限被系统拒绝时，设置不会强制保持开启。

通知触发规则：

- 仅在任务进入 `waiting-user`、`failed` 或 `completed` 时触发。
- 任务停留在同一关键状态期间，后续同类状态事件不重复发送；应用启动或重新加载已有任务时不回放历史通知。
- macOS 下，即使 Agent Island 自身处于前台，也会请求系统以 banner 形式展示通知；如果用户在系统设置中把 Agent Island 通知样式设为“无”或开启专注模式，通知仍可能只进入通知中心列表。
- 通知声音默认使用系统默认通知音，也可选择 Basso、Ping、Glass、Hero、Pop、Sosumi、Tink 或无声；最终是否响铃仍受 macOS 通知设置、专注模式和系统音量影响。
- 关闭“关键状态通知”后，不再发送关键状态通知或声音；选择“无声”时仍发送通知但不设置声音。

...
- 任务完成通知默认关闭。开启后，Agent Island 会请求系统通知权限；通知只包含来源、完成状态和按隐私设置处理后的任务标题，不包含 prompt、回复正文、完整工具输入/输出、完整 shell command、完整 patch 或 transcript 内容。
- 自动确认只归档已经进入 `completed` 的任务，不处理等待用户、失败或暂停任务。
- Claude Code 和 Codex 的真实 hook 接入默认关闭，必须由用户在设置窗口分别打开。未发现本机安装对应工具时，不显示开关，只显示未发现安装。

## 系统通知权限

任务完成通知使用 Tauri notification 插件接入系统通知。用户首次打开“任务完成通知”时，应用请求系统通知权限；权限被系统拒绝时，设置不会强制保持开启。

通知触发规则：

- 仅在任务从非完成态进入 `completed` 时触发。
- 同一完成事件只发送一次。
- 系统通知声音跟随系统通知行为。
- 关闭“任务完成通知”后，不再发送完成通知或声音。
